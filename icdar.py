...
def parse_mat( mat_path):
    ...
    for cell in annot['polygt']:
        x = cell[1][0]
        y = cell[3][0]
        label = cell[4][0] if len(cell[4]) > 0 else '###'
        ...
        polygons.append(pts)
        if label == '#' or label == '*' or label == '###':  # or len(label) <= 2:
            text_tags.append(True)
        else:
            text_tags.append(False)

        # polygon.append(TextInstance(pts, ori, text, config=self.cfg))
    #print("-----polygon-----", polygons)
    return polygons, np.array(text_tags, dtype=np.bool)
